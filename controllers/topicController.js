    ...
    for (let i = 0; i < numberOfArticles; i++) {
      const response = await openAi.writer.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Using the provided user details, generate a single comma-separated string of exactly three compelling contextual article titles related to advancements in {industry/sector}. 
Each title should be a complete, well-structured phrase and should incorporate expert insights, emerging trends, and measurable impacts. 
Ensure each title follows the format: 'Title: Subtitle with Insights or Case Study Reference' with no extra spacing or line breaks. 
Return the output as a single comma-separated string: 'Title 1: Subtitle, Title 2: Subtitle, Title 3: Subtitle`,
          },
        ],
        max_tokens: 100,
        temperature: 0.7,
      ...
      // Remove the escape characters and clean the topics
      rawTopics = rawTopics
        .replace(/\\"/g, '"') // Remove escaped quotes
        .split(",") // Split by commas
        .map((topic) => topic.trim()) // Clean spaces around topics
        .slice(0, 3); // Limit to 3 topics

      const finalTopics = rawTopics.length
        ? rawTopics
        : [
